...
class ServerWorker(object):
    ...
    def process(self, msg):
        ...
        if self.msg_cnt==1:
            ...
            with open("server_private_key_assinatura.txt", "rb") as file2:
                self.server_private_key_assinatura = serialization.load_der_private_key(file2.read(), password=None,
                                                                                        backend=default_backend())

            #vamos fazer a assinatura das duas chaves publicas
            self.signature = self.server_private_key_assinatura.sign(
                self.assinatura,
                padding.PSS(
        ...
        if self.msg_cnt == 2:
            ...
            with open("client_public_key.txt", "rb") as file1:
                self.clientPK = serialization.load_der_public_key(file1.read(), backend=default_backend())


            try:
                self.clientPK.verify(  #
                    self.siga,
                    self.assinatura,
                    padding.PSS(
                        mgf=padding.MGF1(hashes.SHA256()),
